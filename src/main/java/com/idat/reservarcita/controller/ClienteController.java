...
public class ClienteController {
	...
	public ResponseEntity<Void> eliminar(@PathVariable Integer id) {
		ClienteDTOResponse p=service.obtenerById(id);
		if(p != null) {
			service.eliminar(id);
	...
	public ResponseEntity<Void> actualizar(@RequestBody ClienteDTORequest clientes) {
		ClienteDTOResponse p=service.obtenerById(clientes.getId());
	...
	public ResponseEntity<ClienteDTOResponse> obtenerID(@PathVariable Integer id) {
		...
		if(p != null) {			
			return new ResponseEntity<ClienteDTOResponse>(service.obtenerById(id),HttpStatus.OK);
		}
